...
export function padBinaryString(binaryString: string): string {
  ...
  const encodedLength: string = originalLength.toString(2).padStart(8, '0');
...
export function unpadBinaryString(paddedBinaryString: string): string {
  ...
  const originalLength: number = parseInt(paddedBinaryString.slice(-8), 2);
  const start: number = paddedBinaryString.length - 8 - originalLength * 8;
  return paddedBinaryString.slice(start, -8);
}
